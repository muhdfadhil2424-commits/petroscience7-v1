...
/**
 * Predetermined realistic error mapping per student.
 ...
 */
const STUDENT_WRONG_QUESTIONS: Record<string, string[]> = {
  // 🟢 15/15 — 8 MURID (100%)
  'KP-001': [], // Adam Hakimi
  'KP-005': [], // Arif Iman
...
/**
 * Plausible distractor answers based on typical fraction misconceptions.
 */
const WRONG_OPTION_MAP: Record<string, AnswerOption[]> = {
  ...
  CLASS_Q06: ['A', 'C', 'D'],
  CLASS_Q07: ['B', 'A', 'D'],
  CLASS_Q08: ['A', 'B', 'C'], // 'A' (5/14) common error adding denominators
  CLASS_Q09: ['C', 'B', 'D'],
  CLASS_Q10: ['A', 'C', 'D'],
  CLASS_Q11: ['A', 'B', 'D'], // 'A' (2/6) common error (1+1)/(2+4)
  CLASS_Q12: ['A', 'B', 'C'], // 'A' (3/12) common error adding denominators
  CLASS_Q13: ['C', 'D', 'B'], // 'C' (2/4) or 'D' (1/2) without subtracting properly
  CLASS_Q14: ['A', 'D', 'C'],
  CLASS_Q15: ['A', 'D', 'B'],
};

/**
 * Generate the 600 response records for 3 Asah
 */
export function buildDemo600Responses(): {
  ...
  CLASS_3_ASAH_STUDENTS.forEach((st, sIdx) => {
    ...
    INTERACTIVE_CLASS_15_QUESTIONS.forEach((q, qIdx) => {
      ...
      // Find answer text corresponding to letter A=0, B=1, C=2, D=3
      const letterIndex = answerLetter.charCodeAt(0) - 65;
      const answerText = q.options[letterIndex] || answerLetter;

      // Realistic scanning offset: ~4-8 seconds between students per question
...
// Pre-generated pristine demo dataset
const pristineDemo = buildDemo600Responses();
export const DEMO_600_RESPONSES = pristineDemo.records;
export const DEMO_SESSION_NESTED_ANSWERS = pristineDemo.nestedAnswers;

/**
 * Load demo session answers from localStorage or fallback to pristine
 */
export function getStoredDemoSessionAnswers(): Record<string, Record<string, ScannedStudentAnswer>> {
  if (typeof window === 'undefined' || !window.localStorage) {
    return DEMO_SESSION_NESTED_ANSWERS;
  }

  try {
    const raw = window.localStorage.getItem(DEMO_SESSION_STORAGE_KEY);
    if (!raw) {
      // Auto-save pristine demo
      window.localStorage.setItem(DEMO_SESSION_STORAGE_KEY, JSON.stringify(DEMO_SESSION_NESTED_ANSWERS));
      return DEMO_SESSION_NESTED_ANSWERS;
    }
...
/**
 * Reset Demo Data back to the validated 600 responses
 */
export function resetDemoSessionStorage(): Record<string, Record<string, ScannedStudentAnswer>> {
  if (typeof window !== 'undefined' && window.localStorage) {
    window.localStorage.setItem(DEMO_SESSION_STORAGE_KEY, JSON.stringify(DEMO_SESSION_NESTED_ANSWERS));
  }
  return DEMO_SESSION_NESTED_ANSWERS;
...
/**
 * Clear only the Demo Data without touching real session data
 */
