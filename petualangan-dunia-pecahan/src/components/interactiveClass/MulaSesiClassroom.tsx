...
import { AlyaClassroomExplanationModal } from './AlyaClassroomExplanationModal';
import {
...
export const MulaSesiClassroom: React.FC<MulaSesiClassroomProps> = ({
  ...
  // Questions Bank: Exact 15 DSKP 3.1 Questions (Boleh diacak susunan soalan dan pilihan jawapan)
  const [sessionQuestions, setSessionQuestions] = useState<InteractiveClassQuestion[]>(INTERACTIVE_CLASS_15_QUESTIONS);
  const questions: InteractiveClassQuestion[] = sessionQuestions;

  const [currentQuestionIndex, setCurrentQuestionIndex] = useState(0);
  const currentQuestion: InteractiveClassQuestion = questions[currentQuestionIndex] || questions[0];

  // Acak susunan 15 soalan
  const handleShuffleQuestions = () => {
    playSfx('click', soundEnabled);
    const shuffled = shuffleQuestions(sessionQuestions);
    setSessionQuestions(shuffled);
    setCurrentQuestionIndex(0);
    setIsAnswerRevealed(false);
  };

  // Acak pilihan jawapan A/B/C/D bagi soalan
  const handleShuffleOptions = () => {
    playSfx('click', soundEnabled);
    const shuffled = sessionQuestions.map((q) => shuffleQuestionOptions(q));
    setSessionQuestions(shuffled);
    setIsAnswerRevealed(false);
  };
  ...
  const [isAlyaModalOpen, setIsAlyaModalOpen] = useState(false);
  ...
  // Full Screen Camera Scanning Handlers
  ...
  const handleRevealFromScanner = () => {
    // 1. Stop scanning & stop camera & exit fullscreen
    setIsCameraFullScreen(false);
    setIsCameraActive(false);
    // 2. Lock question to prevent new scans while reviewing
  ...
  return (
    <div className="space-y-6">
      {/* ======================================================== */}
      {/* FULL SCREEN CAMERA SCANNER OVERLAY (100vw x 100vh) */}
      {/* ======================================================== */}
      {isCameraFullScreen && isCameraActive && (
        <CameraScannerOverlay
          ...
          onClose={handleBackFromScanner}
          onBackToQuestion={handleBackFromScanner}
          onRevealAnswer={handleRevealFromScanner}
          questionNumber={currentQuestionIndex + 1}
          totalQuestions={questions.length}
          recentScanFeed={recentScanFeed}
      ...
      <div className="bg-white rounded-3xl p-4 sm:p-5 border-2 border-amber-200 shadow-md flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button
            ...
            className="px-3 py-1.5 rounded-xl bg-stone-100 hover:bg-stone-200 text-stone-700 border border-stone-300 text-xs font-bold flex items-center gap-1.5 cursor-pointer transition-colors"
            title="Acak susunan 15 soalan secara rawak"
          >
            <Shuffle className="w-3.5 h-3.5 text-stone-500" />
            <span>Acak Soalan</span>
          </button>
          ...
          <button
            ...
            className="px-3 py-1.5 rounded-xl bg-stone-100 hover:bg-stone-200 text-stone-700 border border-stone-300 text-xs font-bold flex items-center gap-1.5 cursor-pointer transition-colors"
            title="Acak susunan pilihan jawapan A/B/C/D"
          >
            <Shuffle className="w-3.5 h-3.5 text-amber-600" />
            <span>Acak Pilihan</span>
          </button>
          ...
          <div className="relative">
            <button
              ...
              title={`Lompat terus ke soalan 1 - ${questions.length}`}
            >
            ...
            {isQuestionPickerOpen && (
              <div className="absolute right-0 top-full mt-2 w-72 bg-white rounded-2xl p-3 border-2 border-amber-300 shadow-2xl z-50">
                <div className="flex items-center justify-between pb-2 border-b border-stone-100 mb-2">
                  <span className="text-xs font-bold text-[#4A3728]">Pilih Soalan (1 - {questions.length})</span>
                  <button
                    type="button"
          ...
          {/* Mula Imbas Button (Quick Access in Top Bar) */}
          <button
            type="button"
            onClick={handleStartScanning}
            className="px-3.5 py-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 text-white font-bold text-xs shadow-sm flex items-center gap-1.5 cursor-pointer transition-all"
            title="Buka mod camera skrin penuh untuk imbas kad murid"
          >
      ...
      {/* ======================================================== */}
      {/* MAIN CONTAINER: SOALAN SKRIN PENUH & DASHBOARD KELAS */}
      {/* ======================================================== */}
      <div className="max-w-5xl mx-auto w-full space-y-6">
          ...
          <div className="bg-white rounded-3xl p-6 sm:p-7 border-4 border-amber-300 shadow-lg relative">
            ...
            <div className="mt-6 pt-5 border-t border-stone-200 flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={() => {
                  ...
                  className="px-4 py-3 rounded-2xl bg-stone-100 hover:bg-stone-200 text-stone-700 text-xs font-bold disabled:opacity-40 flex items-center gap-1.5 cursor-pointer transition-colors"
                >
                ...
                <button
                  ...
                  className={`px-3.5 py-3 rounded-2xl text-xs font-black shadow-xs flex items-center gap-1.5 cursor-pointer transition-all ${
                    isQuestionLocked
              ...
              {/* Primary Action Buttons: Mula Imbas & Tunjuk Jawapan */}
              <div className="flex items-center gap-2.5 sm:gap-3 flex-wrap">
                {/* 📷 MULA IMBAS (PRIMARY TEACHER ACTION) */}
                <button
                  type="button"
                  id="btn-mula-imbas-primary"
                  onClick={handleStartScanning}
                  className="px-6 sm:px-8 py-3 rounded-2xl bg-emerald-600 hover:bg-emerald-500 active:bg-emerald-700 text-white font-black text-sm sm:text-base shadow-lg hover:shadow-xl flex items-center justify-center gap-2.5 cursor-pointer transition-all hover:scale-[1.02] ring-4 ring-emerald-300/40"
                  title="Buka mod camera skrin penuh untuk imbas kad murid"
                >
                  <Camera className="w-5 h-5" />
                  <span>📷 MULA IMBAS</span>
                  {stats.totalAnswered > 0 && (
                    <span className="text-xs bg-emerald-800 text-emerald-100 px-2 py-0.5 rounded-full font-mono font-bold">
                ...
                <button
                  ...
                  className={`px-5 py-3 rounded-2xl text-xs sm:text-sm font-black shadow-md flex items-center gap-2 cursor-pointer transition-all ${
                    isAnswerRevealed
                ...
                {currentQuestionIndex < questions.length - 1 ? (
                  <button
                    type="button"
                    onClick={handleNextQuestion}
                    className="px-4 py-3 rounded-2xl bg-[#3c4233] hover:bg-stone-900 text-amber-300 text-xs font-black flex items-center gap-1.5 cursor-pointer shadow-md transition-all"
                  >
                  ...
                  <button
                    type="button"
                    onClick={handleNextQuestion}
                    className="px-4 py-3 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-black flex items-center gap-1.5 cursor-pointer shadow-md transition-all animate-bounce"
                  >
          ...
          <div className="bg-white rounded-3xl p-5 sm:p-6 border-2 border-amber-200 shadow-md">
            <div className="flex items-center justify-between text-sm sm:text-base font-black mb-2">
              <span className="text-emerald-700 flex items-center gap-2">
                ...
                <span>🟢 {stats.totalAnswered} / {stats.totalStudents} MURID SUDAH JAWAB</span>
              </span>
              <span className="font-mono text-stone-700 font-black">
                {stats.percentAnswered}%
              </span>
            </div>
            <div className="w-full bg-stone-100 rounded-full h-4 sm:h-5 overflow-hidden border border-stone-200 p-0.5">
              <div
          ...
          {/* ======================================================== */}
          {/* 2-COLUMN SECTION: TABURAN JAWAPAN & STATUS KAD MURID */}
          {/* ======================================================== */}
            ...
            >
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-white p-4 rounded-2xl border-2 border-amber-200 text-center shadow-xs">
                  ...
                  <span className="text-2xl font-black text-[#4A3728] font-mono">{questions.length}</span>
                </div>
      ...
      {/* 💗 ALYA TERANGKAN MODAL */}
      <AlyaClassroomExplanationModal
        isOpen={isAlyaModalOpen}
        onClose={() => setIsAlyaModalOpen(false)}
