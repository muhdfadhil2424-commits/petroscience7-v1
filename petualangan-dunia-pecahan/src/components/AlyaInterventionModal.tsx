      ...
      <AnimatePresence>
        <div className="fixed inset-0 z-[1350] flex items-center justify-center p-3 sm:p-5 bg-slate-950/80 backdrop-blur-md overflow-y-auto animate-fadeIn">
          <motion.div
            ...
            className="bg-[#FFFDF7] text-[#4A3728] rounded-3xl p-5 sm:p-7 border-4 border-[#F4C95D] shadow-2xl max-w-2xl w-full my-auto space-y-4.5 max-h-[92vh] overflow-y-auto relative"
          >
            {/* ======================================================== */}
            ...
            {/* ======================================================== */}
            <div className="bg-white p-4 rounded-2xl border-2 border-indigo-100 shadow-2xs">
              <div className="flex items-center justify-between flex-wrap gap-2.5">
                <div>
                  ...
                  <div className="text-xs text-stone-500 font-medium ml-7 mt-0.5">
                    Kelas: <strong className="text-stone-800">{plan.studentClass}</strong>
                    <span className="ml-2 text-stone-400 font-mono">({plan.studentId})</span>
                  </div>
                </div>

                <div className="flex items-center gap-3">
                  <div className="bg-stone-50 px-3 py-1.5 rounded-xl border border-stone-200 text-right">
                  ...
                  <div className="bg-indigo-50 px-3 py-1.5 rounded-xl border border-indigo-200 text-right">
                    <span className="text-[10px] uppercase font-bold text-indigo-800 block">Kecenderungan:</span>
            ...
            {/* ======================================================== */}
            <div className="bg-gradient-to-r from-amber-50 via-indigo-50/50 to-amber-50 p-4 rounded-2xl border-2 border-amber-200 shadow-2xs space-y-1.5">
              <div className="flex items-center gap-2">
              ...
              <p className="text-xs text-stone-800 font-medium leading-relaxed bg-white/80 p-3 rounded-xl border border-amber-100">
                “{plan.alyaSummary}”
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div className="bg-white p-3.5 rounded-2xl border-2 border-emerald-200 shadow-2xs space-y-2">
                ...
                <ul className="space-y-1.5 text-xs text-stone-700">
                  {plan.strengths.map((str, idx) => (
                    <li key={idx} className="flex items-start gap-1.5 bg-emerald-50/60 p-2 rounded-xl border border-emerald-100">
              ...
              {/* Perlu Pengukuhan */}
              <div className="bg-white p-3.5 rounded-2xl border-2 border-rose-200 shadow-2xs space-y-2">
                ...
                <ul className="space-y-1.5 text-xs text-stone-700">
                  {plan.weaknesses.map((wk, idx) => (
                    <li key={idx} className="flex items-start gap-1.5 bg-rose-50/60 p-2 rounded-xl border border-rose-100">
            ...
            {/* ======================================================== */}
            <div className="bg-gradient-to-br from-indigo-900 to-indigo-950 text-white p-4.5 rounded-2xl shadow-md border-2 border-indigo-700 space-y-2">
              <div className="flex items-center justify-between flex-wrap gap-2 pb-1.5 border-b border-indigo-700/60">
                ...
                <span className="text-[10px] font-bold bg-indigo-800/80 px-2 py-0.5 rounded-full text-indigo-200 border border-indigo-600">
                  Kecenderungan + Kelemahan DSKP
                </span>
              </div>
              <p className="text-xs font-medium leading-relaxed text-indigo-100 bg-white/10 p-3 rounded-xl border border-white/15">
            ...
            <div className="space-y-2.5">
              ...
              <div className="space-y-3">
                {plan.approachOrder.map((mode, index) => {
                  if (mode === 'visual') {
                    return (
                      <div key="visual" className="bg-white p-3.5 rounded-2xl border-2 border-blue-200 shadow-2xs space-y-2">
                        <div className="flex items-center justify-between pb-1 border-b border-blue-100">
                          <span className="text-xs font-black text-blue-950 flex items-center gap-1.5">
                            <span>👀</span>
                            <span>A. PENDEKATAN VISUAL</span>
                          </span>
                          <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-blue-50 text-blue-800 border border-blue-200">
                            Keutamaan #{index + 1}
                          </span>
                        </div>
                        <ul className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 text-[11px] text-stone-700">
                          {plan.visualApproach.activities.map((act, aIdx) => (
                            <li key={aIdx} className="bg-stone-50 p-2 rounded-xl border border-stone-200 flex items-start gap-1.5">
                              <span className="text-blue-600 font-bold shrink-0">•</span>
                              <span>{act}</span>
                            </li>
                          ))}
                        </ul>
                        <div className="bg-blue-50/70 p-2.5 rounded-xl border border-blue-100 text-xs text-blue-950">
                          <strong>Contoh:</strong> “{plan.visualApproach.example}”
                        </div>
                      </div>
                    );
                  }
                  ...
                  if (mode === 'kinesthetic') {
                    return (
                      <div key="kinesthetic" className="bg-white p-3.5 rounded-2xl border-2 border-emerald-200 shadow-2xs space-y-2">
                        <div className="flex items-center justify-between pb-1 border-b border-emerald-100">
                          <span className="text-xs font-black text-emerald-950 flex items-center gap-1.5">
                            <span>🖐️</span>
                            <span>B. PENDEKATAN KINESTETIK</span>
                          </span>
                        ...
                        <div className="bg-emerald-50/70 p-2.5 rounded-xl border border-emerald-100 text-xs text-emerald-950">
                          <strong>Contoh:</strong> “{plan.kinestheticApproach.example}”
                        </div>
                      </div>
                    );
                  }
                  ...
                  if (mode === 'auditory') {
                    return (
                      <div key="auditory" className="bg-white p-3.5 rounded-2xl border-2 border-amber-200 shadow-2xs space-y-2">
                        <div className="flex items-center justify-between pb-1 border-b border-amber-100">
                          <span className="text-xs font-black text-amber-950 flex items-center gap-1.5">
                            <span>🎧</span>
                            <span>C. PENDEKATAN AUDITORI</span>
                          </span>
                        ...
                        <div className="bg-amber-50/70 p-2.5 rounded-xl border border-amber-100 text-xs text-amber-950">
                          <strong>Contoh:</strong> “{plan.auditoryApproach.example}”
                        </div>
                      </div>
                    );
                  }
