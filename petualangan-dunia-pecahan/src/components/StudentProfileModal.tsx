...
import { User, School, Sparkles, UserCheck, ShieldAlert, ArrowRight, Lock, X, LogOut } from 'lucide-react';
...
export const StudentProfileModal: React.FC<StudentProfileModalProps> = ({
  ...
  const handleStartPlaying = (e: React.FormEvent) => {
    ...
    if (isSelectExistingMode) {
      ...
      const existing = allStudents.find((s) => s.id === selectedExistingStudentId);
      if (existing) {
  ...
  return (
    <div className="fixed inset-0 z-[1050] flex items-center justify-center p-3 sm:p-6 bg-slate-950/75 backdrop-blur-md overflow-y-auto">
      ...
      >
          ...
          <div className="flex items-center gap-1.5 flex-wrap justify-end">
            ...
            {onOpenKelasInteraktif && (
              <button
                ...
                className="flex items-center gap-1 px-2.5 py-1.5 rounded-xl bg-[#D98262] hover:bg-[#c26f51] text-white text-xs font-bold border border-[#b85b3b] shadow-sm transition-all cursor-pointer"
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-[#3c4233] hover:bg-[#2d3226] text-amber-200 text-xs font-bold border border-[#525a46] shadow-sm transition-all cursor-pointer"
            ...
            {/* Optional Close button if student already logged in */}
            {currentStudent && onClose && (
              <button
                ...
                className="p-1.5 rounded-xl bg-amber-100/80 hover:bg-amber-200 text-[#4A3728] transition-colors cursor-pointer"
                title="Tutup"
              >
                <X className="w-4 h-4" />
              </button>
        ...
        {/* Welcome Mascot & Title - Alya does NOT appear here */}
        <div className="flex flex-col items-center text-center space-y-1.5 mb-4">
          <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl bg-gradient-to-br from-amber-100 to-amber-200/90 border-2 border-[#F4C95D] shadow-md flex items-center justify-center relative text-[#D98262]">
            <User className="w-7 h-7 sm:w-8 sm:h-8 text-[#D98262]" />
          ...
          {currentStudent && onLogout && (
            <button
              ...
              className="mt-1 text-xs text-red-700 hover:text-red-900 font-bold underline cursor-pointer flex items-center gap-1"
            >
              <LogOut className="w-3 h-3" />
              <span>Log keluar dari akaun {currentStudent.nama}</span>
            </button>
          )}
        </div>
        ...
        {allStudents.length > 0 && (
          <div className="flex items-center justify-center gap-2 mb-3">
            ...
            >
              👦 Pilih Sedia Ada ({allStudents.length})
            </button>
          </div>
        )}
        ...
        <form onSubmit={handleStartPlaying} className="space-y-3.5">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            type="submit"
            className="w-full py-3.5 rounded-2xl text-white font-bold text-base sm:text-lg shadow-xl border-2 border-white flex items-center justify-center gap-2 cursor-pointer mt-2 bg-gradient-to-r from-[#D98262] to-[#c87253]"
          >
        ...
        {/* Dedicated Teacher Section */}
        {onOpenKelasInteraktif && (
          <div className="mt-4 pt-3 border-t border-amber-200/80 flex items-center justify-between gap-2 bg-white/70 p-3 rounded-2xl border border-amber-200">
            <div className="flex items-center gap-2 text-left">
              <span className="text-xl">👨‍🏫</span>
              <div>
                <span className="text-xs font-bold text-[#4A3728] block leading-tight">
                  Pilihan Khas Guru: Mode Kelas Interaktif
                </span>
                <span className="text-[10px] text-stone-500 font-medium">
                  1 Laptop Guru • Kad Jawapan Personal Murid
                </span>
              </div>
            </div>
            <button
              ...
              className="px-3 py-1.5 rounded-xl bg-[#3c4233] hover:bg-[#2c3125] text-amber-200 text-xs font-bold shadow-sm transition-all cursor-pointer whitespace-nowrap"
            >
              Buka Sekarang ➔
            </button>
          </div>
        ...
        <p className="text-[11px] text-gray-500 text-center font-medium mt-3">
          Data kemajuan peribadi murid akan disimpan secara selamat bagi setiap sesi permainan.
        </p>
      </motion.div>
    </div>
  );
